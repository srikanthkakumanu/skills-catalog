...
A Spring Boot 4.x / Java 27 reference collection for reactive concerns, promoted from
...
|---|---|---|
| `webflux-reactive-patterns` | WebFlux controllers, Reactor pipelines, R2DBC persistence, streaming, backpressure, cancellation. | `webflux-reactive-patterns/` |
...
topic path here (`skills/reactive/<topic>/`) when generating project conventions.
...
./install.sh --skill reactive

# Install via file copy
./install.sh --skill reactive --mode copy

# Install to a specific runtime
./install.sh --skill reactive --target claude

# Force-reinstall
./install.sh --skill reactive --force
```
...
- **`webflux-reactive-patterns/`** — `SKILL.md` (directive), `examples/` (bad vs good), `templates/`/`agents/` where present
