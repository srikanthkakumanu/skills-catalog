...
## This Catalog's Gradle Convention

*Added by skills-catalog — not part of the upstream skill above. Build tool and Java version are
this catalog's house standard (Gradle Groovy DSL, Java 27 — see `../../../conventions/`), not
upstream guidance.*
...
spring:
  jpa:
    hibernate:
      ddl-auto: validate   # schema owned by Flyway, not Hibernate — see flyway-migrations
    properties:
      hibernate:
...
class OrderRepositoryTest {
    ...
    void should_create_and_retrieve_order() {
        Order saved = repository.save(Order.create(UUID.randomUUID()));
        assertThat(repository.findById(saved.getId())).isPresent();
    }
}
```

Migration file conventions (naming, safe schema-change patterns) belong to `flyway-migrations` in
`02-persistence/` — not duplicated here.
