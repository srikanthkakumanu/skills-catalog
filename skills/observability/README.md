...
**Observability reference for Spring Boot 4: service-wide Actuator/Micrometer/OTLP telemetry, plus Spring AI-specific model observability.**

## Overview

A Spring Boot 4.x / Java 27 reference collection for observability concerns, promoted from
...
|---|---|---|
| `production-observability` | Actuator, Micrometer Observation & Tracing, OpenTelemetry/OTLP export, structured logging, health probes, dashboards, alerts. | `production-observability/` |
| `ai-observability` | Spring AI model observations, token usage, latency, cost attribution, prompt/completion logging. | `ai-observability/` |
...
topic path here (`skills/observability/<topic>/`) when generating project conventions.
...
./install.sh --skill observability

# Install via file copy
./install.sh --skill observability --mode copy

# Install to a specific runtime
./install.sh --skill observability --target claude

# Force-reinstall
./install.sh --skill observability --force
```
...
- **`production-observability/`** — `SKILL.md` (directive), `examples/` (bad vs good), `templates/`/`agents/` where present
- **`ai-observability/`** — `SKILL.md` (directive), `examples/` (bad vs good), `templates/`/`agents/` where present
