...
2. **Style ADR** — monolith / modular monolith / microservices / event-driven / serverless, or a named split. Checklist: independent scaling needs? single team/deploy cadence? explicit "avoid over-engineering" signal? bursty/event-triggered work?
...
4. Present both ADRs at `status: pending`. Wait for confirmation. Update to `confirmed` (or revise and re-present) per the response.
