...
A Spring Boot 4.x / Java 27 reference collection for testing concerns, promoted from
...
|---|---|---|
| `testing-pyramid` | Test structure, naming conventions, Mockito patterns, @WebMvcTest, @DataJpaTest, Testcontainers setup. | `testing-pyramid/` |
...
topic path here (`skills/testing/<topic>/`) when generating project conventions.
...
./install.sh --skill testing

# Install via file copy
./install.sh --skill testing --mode copy

# Install to a specific runtime
./install.sh --skill testing --target claude

# Force-reinstall
./install.sh --skill testing --force
```
...
- **`testing-pyramid/`** — `SKILL.md` (directive), `examples/` (bad vs good), `templates/`/`agents/` where present
